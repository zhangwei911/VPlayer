...
import viz.commonlib.JWtDecoderForAndroid
import viz.commonlib.JwkForAndroid
        ...
        getRSAPublicKeyByKidAsyn(decoder.keyId, object : ICallBack {
            override fun onSuccess() {
                try {
                    ...
                    if (decoder.issuer != ID_TOKEN_ISSUE) {
                        callBack.onFailed()
                        return
                    }
                    ...
                    l.d(
                        "Exception captured:" + e.javaClass.simpleName
                    )
                    callBack.onFailed()
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    try {
                        ...
                        l.d("parse JsonArray failed." + e.message)
                        iCallBack.onFailed()
                    } catch (e: JSONException) {
                        l.d("parse JsonArray failed." + e.message)
                        iCallBack.onFailed()
