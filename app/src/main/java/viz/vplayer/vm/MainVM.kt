...
//        state.set(KEY_SEARCH_RESULT, searchBeanList)
                    ...
                    .enqueue(VCallback<ResponseBody>(onResult = { call, response, resultResponseBody ->
                        try {
                            ...
                            lis.forEachIndexed { index, element ->
                                searchHtmlResultBean.apply {
                                    try {
                                        val nameElements =
                                            element.select(nameCss)
                                        val nameElement =
                                            nameElements[nameIndex]
                                        val name = if (isNameAttr) {
                                            nameElement.attr(nameAttr)
                                        } else {
                                            nameElement.html()
                                        }
                                        val imgElements =
                                            element.select(imgCss)
                                        val imgElement = imgElements[imgIndex]
                                        var img = if (isImgAttr) {
                                            imgElement.attr(imgAttr)
                                        } else {
                                            imgElement.html()
                                        }
                                        if (img.startsWith("//")) {
                                        ...
                                        val descElements =
                                            element.select(descCss)
                                        val descElement =
                                            descElements[descIndex]
                                        val desc = if (isDescAttr) {
                                            descElement.attr(descAttr)
                                        } else {
                                            descElement.html()
                                        }
                                        val urlElements =
                                            element.select(urlCss)
                                        val urlElement = urlElements[urlIndex]
                                        val url = if (isUrlAttr) {
                                            urlElement.attr(urlAttr)
                                        } else {
                                            urlElement.html()
                                        }
                    ...
                    .enqueue(VCallback<ResponseBody>(onResult = { call, response, resultResponseBody ->
                        try {
                            ...
                            if (episodesBean.isReg) {
                                ...
                                lis?.forEachIndexed { index, element ->
                                    if(episodesBean.isMultiList){
                                        val listInner = element.select(episodesBean.listCssMulti)
                                        listInner?.forEachIndexed { indexInner, elementInner ->
                                            addEpisode(elementInner, episodesBean, uri, episodeList)
                                        }
                                    }else {
                                        addEpisode(element, episodesBean, uri, episodeList)
                                    }
                                }
                            }
    ...
    private fun addEpisode(
        ...
        uri: Uri,
        episodeList: MutableList<String>
    ) {
